...
const IAMGE = "https://www.ipfs.io/ipfs/Qmhttps://ipfs.io/ipfs/QmTYEboq8raiBs7GTUg2yLXB3PMz6HuBNgNfSZBx5Msztg/shoes.jpg";
...
describe("Dappazon", () => {
  ...
  describe("Listing", () => {
    ...
    beforeEach(async () => {
      // List an item
      transaction = await dappazon.connect(deployer).list(ID, NAME, CATEGORY, IAMGE, COST, RATING, STOCK);
  ...
  describe("Widhrawing", () => {
    ...
    beforeEach(async () => {
      // List an item
      let transaction = await dappazon.connect(deployer).list(ID, NAME, CATEGORY, IAMGE, COST, RATING, STOCK);
      await transaction.wait();
      ...
      // Get deployer's balance before withdrawing
    ...
    it("Updates the owner balance", async () => {
      const balanceAfter = await ethers.provider.getBalance(deployer.address); // Get the balance of the contract; Check the smart contract balance
      expect(balanceAfter).to.be.greaterThan(balanceBefore); // Check if the contract balance is equal to 0
    });

    it("Updates the contract balance", async () => {
      const result = await ethers.provider.getBalance(dappazon.address); // Get the balance of the contract; Check the smart contract balance
      expect(result).to.equal(0); // Check if the contract balance is equal to the cost of the item
    });
  });
});
